...
struct Offset:
    ...
    fn __init__(inout self, hour: UInt8, minute: UInt8, sign: Int8):
        ...
        debug_assert(
            hour < 100 and hour >= 0 and minute < 100 and minute >= 0 and (sign == 1 or sign == -1),
            "utc offsets can't have a member bigger than 100, and sign must be either 1 or -1",
        )
    ...
    fn get(self, key: StringLiteral) -> OptionalReg[ZoneDST]:
        ...
        try:
            ...
            with open(self._file, "rb") as f:
                ...
                value = (int(bufs[0]) << 24) | (int(bufs[1]) << 16) | (int(bufs[2]) << 8) | int(bufs[3])
...
struct Leapsecs:
    ...
    fn __eq__(self, value: Tuple[UInt16, UInt8, UInt8]) -> Bool:
        ...
        """
        return self.year == value[0] and self.month == value[1] and self.day == value[2]
...
](owned timezones: List[StringLiteral] = List[StringLiteral]()) -> Optional[ZoneInfo[T, A]]:
