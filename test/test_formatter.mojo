...
def _loud_styles() -> Styles:
    ...
    var levels: Dict[Int, mist.Style] = {
        Level.TRACE._value: base.foreground(0xD48244),
        Level.DEBUG._value: base.foreground(0xD4317D),
        Level.INFO._value: base.foreground(0xD48244),
        Level.WARNING._value: base.foreground(0xDECF2F),
        Level.ERROR._value: base.foreground(0x13ED84),
        Level.CRITICAL._value: base.foreground(0xBD37DB),
    }
...
def test_apply_styles_defaults_on_for_the_default_formatter() raises:
    ...
    var logger = BoundLogger(PrintLogger[Level.DEBUG]())
...
def test_apply_styles_can_be_forced_on() raises:
    ...
    var logger = BoundLogger(PrintLogger[Level.DEBUG](), formatter=JSON_FORMATTER[pretty=False], apply_styles=True)
    assert_true(logger.apply_styles)


def test_apply_styles_can_be_forced_off() raises:
    """An explicit `False` overrides the formatter's preference."""
    var logger = BoundLogger(PrintLogger[Level.DEBUG](), formatter=DEFAULT_FORMATTER, apply_styles=False)
    assert_false(logger.apply_styles)
...
def test_custom_formatter_declares_its_own_preference() raises:
    ...
    assert_false(BoundLogger(PrintLogger[Level.DEBUG](), formatter=unstyled).apply_styles)
    assert_true(BoundLogger(PrintLogger[Level.DEBUG](), formatter=styled).apply_styles)
...
def test_apply_styles_survives_a_bind() raises:
    ...
    var parent = BoundLogger(PrintLogger[Level.DEBUG](), formatter=JSON_FORMATTER[pretty=False])
